...
//   const jobData = messages?.job_page?.jobs || [];
...
//     const selectedJob = jobData.find(job => job.slug === slug);
...
export default function Jobpages({ params }) {
  ...
  const jobs = careersData?.JobListSection?.jobs || [];
  const applySection = messages?.job_page?.ApplySection; 

  if (!jobs.length) return <div>Job page not found</div>; 
  ...
  return (
    <div className="">
      ...
      <div className='w-full max-w-[1220px] mx-auto'>
        <ApplySection data={applySection} />
      </div>
    </div>
  );
}